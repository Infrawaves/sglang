...
use crate::config::{
    ...
    default_stale_request_timeout_secs, resolve_mode, ActiveLoadConfig, AffinityConfig,
...
pub struct Cli {
    ...
    /// offloaded as `input_ids`. Rejected with `--policy cache_aware`
    ...
    pub fn into_config(self) -> Result<Config> {
        ...
        if self.no_tokenizer {
            ...
            if self.policy == PolicyKind::CacheAware {
                return Err(anyhow!(
                    "--policy cache_aware requires a tokenizer (it routes on prompt \
...
mod tests {
    ...
    /// cache_aware routes on prompt token hashes, so it cannot run without
