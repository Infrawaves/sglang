...
- **`--hicache-ratio HICACHE_RATIO`**: The ratio of the size of host KV cache memory pool to the size of device pool.
  For example, a value of 2 means the host memory pool is twice as large as the device memory pool. Values below 1 are supported when a smaller L2 working set is desired; they reduce the L2 hit rate and make individual transfers more likely to exceed available host pages.

- **`--hicache-size HICACHE_SIZE`**: The size of host KV cache memory pool in gigabytes. This parameter overrides `hicache-ratio` if set.
  For example, `--hicache-size 30` allocates 30GB (1GB = 1e9 bytes) for the host memory pool **for each rank**. If there are 8 ranks, then the total memory size is 240GB. A smaller value is valid, but a request or retraction span must still fit the page-aligned L2 capacity when it is transferred.
